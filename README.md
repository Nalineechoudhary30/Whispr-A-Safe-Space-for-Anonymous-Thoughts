...
### UI/Styling
- Tailwind CSS with ShadCN UI for a clean, modern, and accessible component library.
...
### Generative AI
- Google's Genkit integrated with Gemini models to power all AI features, from message classification to empathetic chat responses.
...
**Process:**  
- The user's message content is sent to this flow.  
- A prompt instructs the Gemini model to act as an "AI Guardian" and classify the message into one of three categories: normal, stressed, or need_help.  
- The model returns a JSON object containing the aiLabel and a aiConfidence score.  
- This data is saved along with the whisper in the /posts collection.
...
**Process:**  
- The content of the selected whisper is sent to the flow.  
- The prompt instructs the model to act as an empathetic friend and generate a concise, validating message. It is explicitly told not to give medical advice.  
- The generated reply is then displayed to the user in a dialog or attached to the post for the admin's review.
...
**Process:**  
- The user's message is sent to the flow.  
- A sophisticated prompt defines a dual personality for the AI:  
...
- The prompt also includes a rule for escalation. If the AI detects severe distress or a crisis, it sets an escalate boolean flag to true.  
- The AI's generated text response and the escalate flag are returned. If escalate is true, the UI displays the helpline resources panel.
